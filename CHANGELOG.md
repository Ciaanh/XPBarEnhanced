...
- **New Sigil bar style**: a class ring that advances through four tiers as you approach the level cap — a plain band, then studs and cardinal mounts, then a fuller setting, then a crowned frame. The casing, rune band and class crest are tinted with your class colour, the centre carries level / percentage / time-to-level, and a companion ring tracks the secondary source. Five options — skin, size, tier mode, pinned tier, and class-colour tint — live under Visual and read "Sigil only" under any other style.
- **30 new textures for the Sigil style**: 4 tier casings, 13 per-class rune bands and 13 class crests, all greyscale and tinted at runtime. They are generated by `assets/raw/generate_sigil.py` (dev-only, not shipped), so the art is reproducible from source and every mark is drawn from primitives rather than derived from existing artwork.
