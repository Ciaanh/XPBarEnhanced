...
- **New Sigil bar style**: a class ring that advances through four tiers as you approach the level cap — a plain band, then pointed cardinal fittings, then finials and filigree, then a crowned frame with eight fittings. XP fills as a **clockwise arc** set into the frame itself, running in a channel between the casing's outer band and its inner rim, emerging at the class crest's left edge and completing at its right; quest and rested extents continue the same arc in their own colours. The casing is themed to your class family — a Mage's frame and a Warrior's frame differ in metalwork, not just tint — and casing and crest are tinted with your class colour, the centre carries level / percentage / time-to-level, and a companion ring tracks the secondary source with the same arc. Five options — skin, size, tier mode, pinned tier, and class-colour tint — live under Visual and read "Sigil only" under any other style.
- **43 new textures for the Sigil style**: 28 tier casings (a neutral set plus six class-family themes — arcane, sacred, wild, martial, umbral and draconic, each with its own fittings, crown finials and filigree), 13 class crests as round medallions carrying each class's iconic motif (crossed swords, warhammer, drawn bow, winged orb, crossed warglaives, ...), and the arc's fill and track annuli, all greyscale and tinted at runtime. They are generated by `assets/raw/generate_sigil.py` (dev-only, not shipped), so the art is reproducible from source and every mark is drawn from primitives rather than derived from existing artwork. (An earlier cut shipped per-class rune bands; they were removed — two concentric rings of repeated marks read as noise, and the tier signal belongs to the frame.)
- `/xpbe sigil debug` prints the tier, skin and accent the resolver picks across a level ladder at two level caps, so the tier bands can be checked without levelling.
- `/xpbe test tier` previews the tier-ascend flourish without levelling and without touching saved tier state.
...
- **The style gallery's third row of swatches painted over the option rows below it.** The gallery grew its swatch grid for a ninth style but not the row frame the options panel stacks from, so the Sigil swatch overlapped "Show minimap button" and "Show milestone ticks". The row now grows with the grid.
