...
| | |
|---|---|
| **Status** | implemented (2026-08-01) as `ui/styles/sigil/` + 30 generated TGA, pending in-game validation |
...
| **Effort** | Medium |
| **Reuse** | The orb's shape mechanism, not the circular's: a VERTICAL StatusBar clipped by one circle `MaskTexture`, so animation, flash, colours, tooltips and max-level repurpose all arrive from the mixins unchanged. Tier art swaps textures on the one cached frame — no frame per skin, no frame per tier |
| **Notes** | Four tiers as fractions of `GetMaxPlayerLevel()` (0.00/0.25/0.55/0.85), never literal levels, so the bands track the cap across expansions. Resolution is a pure-data module (`SigilSkins.lua`) with no widget calls, inspectable via `/xpbe sigil debug`. The tier table is sparse and `Resolve` walks down, so art can land one tier at a time; with no art defined it falls back to the procedural **Halo**, which is also the forced look whenever the bar is not tracking XP (max-level repurpose, companion bar). Halo is internal and never listed in the skin dropdown. The tier *swap* rides `RenderBar`, deliberately **not** the celebration path, so an animations-off player is not stranded on stale art; only the *ascend animation* hangs off `OnLevelUpCelebration`. **Blocked on:** nothing — the `timeReadout` capability that the centre ETA needed shipped in the same release. **Art:** all 30 TGA are generated by `assets/raw/generate_sigil.py` (4 casings, 13 per-class rune bands, 13 crests; ~1.6 MB, greyscale, tinted at runtime). Parametric rather than hand-drawn, so tier pacing or bevel weight is one edit and one command, and nothing is derived from existing artwork. **Remaining:** in-game validation, and the open question of whether the accent tint should reach the casing or stop at the runes and crest. The crests are abstract heraldic marks by necessity at 64px, not renditions of the official class crests — worth revisiting if someone wants to hand-draw them |
